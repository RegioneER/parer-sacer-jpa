...
public class AroVersIniUnitaDoc implements Serializable {
    ...
    private List<AroVersIniArchivSec> aroVersIniArchivSecs;
    private List<AroVersIniLinkUnitaDoc> aroVersIniLinkUnitaDocs;
    private List<AroVersIniDoc> aroVersIniDocs;
    private List<AroVersIniDatiSpec> aroVersIniDatiSpecs;

    public AroVersIniUnitaDoc() {
    }

    @Id
    @SequenceGenerator(name = "ARO_VERS_INI_UNITA_DOC_IDVERSINIUNITADOC_GENERATOR", sequenceName = "SARO_VERS_INI_UNITA_DOC", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "ARO_VERS_INI_UNITA_DOC_IDVERSINIUNITADOC_GENERATOR")
